...
class PromptLoader:
    ...
    def load_configured_prompt(
        ...
        if configured_path is None:
            raise PromptLoaderError(
                f"Prompt configuration variable is missing: {configuration_key}"
            )
        if not isinstance(configured_path, str) or not configured_path.strip():
            raise PromptLoaderError(f"Prompt configuration variable is blank: {configuration_key}")
    ...
    def load_prompt(self, prompt_path: Path | str, variables: Mapping[str, object]) -> str:
        ...
        if unresolved:
            raise PromptLoaderError(f"Unresolved prompt variables: {', '.join(unresolved)}")
    ...
    def _load_template(self, prompt_path: Path) -> str:
        ...
        if prompt_path.suffix.lower() != ".md":
            raise PromptLoaderError(f"Prompt file must be a Markdown (.md) file: {prompt_path}")
        if not prompt_path.exists():
            raise PromptLoaderError(f"Prompt file does not exist: {prompt_path}")
        if not prompt_path.is_file():
            raise PromptLoaderError(f"Prompt path is not a file: {prompt_path}")
        ...
        with self._cache_lock:
            ...
            try:
                ...
                raise PromptLoaderError(f"Unable to read prompt file: {resolved_path}") from error
            if not template.strip():
                raise PromptLoaderError(f"Prompt file is empty: {resolved_path}")
